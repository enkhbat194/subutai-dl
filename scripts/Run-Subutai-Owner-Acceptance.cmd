...
set "IMPERSONATION_RETRY_SCRIPT=%SCRIPT_DIR%owner-youtube-impersonation-retry.ps1"
...
:try_ua_retry
if not exist "%UA_RETRY_SCRIPT%" goto :try_impersonation_retry
echo.
...
:try_impersonation_retry
if not exist "%IMPERSONATION_RETRY_SCRIPT%" goto :try_wpc_retry
echo.
echo Browser cookie routes did not pass. Trying packaged Chrome request impersonation with owner browser sessions...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%IMPERSONATION_RETRY_SCRIPT%" %*
set "EXIT_CODE=%ERRORLEVEL%"
if "%EXIT_CODE%"=="0" goto :passed
...
echo.
echo Browser impersonation routes did not pass. Trying packaged browser-minted WPC PO-token fallback...
