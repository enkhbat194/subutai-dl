...
foreach ($candidate in $browserCandidates) {
  ...
  Write-Host "Trying owner-network YouTube acceptance with packaged runtime; browser=$candidate"
  $exitCode = Invoke-YtDlpAttempt -YtDlp $ytDlp -Arguments $arguments -StdoutPath $stdout -StderrPath $stderr
  if ($exitCode -ne 0) {
    $failures.Add("browser=$candidate exit=$exitCode :: $(Read-Tail $stderr)")
    continue
  }
  ...
  if (-not $media -or -not (Test-PlayableMedia -Path $media.FullName -Ffprobe $ffprobe)) {
    $failures.Add("browser=$candidate produced no valid playable media output")
    continue
  }
...
throw "Subutai owner-network YouTube acceptance failed for all candidates.`n$($failures -join "`n")"
