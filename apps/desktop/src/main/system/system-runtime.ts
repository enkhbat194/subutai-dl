...
function setUpdate(next: Partial<UpdateState>): void {
  update = {
    ...update,
    ...next,
    currentVersion: app.getVersion(),
...
async function checkForUpdates(): Promise<SystemState> {
  if (!app.isPackaged) {
    setUpdate({ status: 'disabled', checkedAt: new Date().toISOString(), error: 'Шинэчлэлт зөвхөн суулгасан хувилбар дээр шалгагдана.' });
    return state();
  }
  setUpdate({ status: 'checking', checkedAt: new Date().toISOString(), error: undefined });
  try {
...
async function downloadUpdate(): Promise<SystemState> {
  ...
  try {
    setUpdate({ status: 'downloading', error: undefined });
...
function configureUpdater(): void {
  ...
  autoUpdater.on('checking-for-update', () => setUpdate({ status: 'checking', error: undefined }));
  autoUpdater.on('update-available', (info) => {
    setUpdate({ status: 'available', availableVersion: info.version, checkedAt: new Date().toISOString(), error: undefined });
  });
  autoUpdater.on('update-not-available', () => {
    setUpdate({ status: 'not-available', availableVersion: undefined, checkedAt: new Date().toISOString(), error: undefined });
  });
  autoUpdater.on('download-progress', (progress) => {
    setUpdate({ status: 'downloading', progressPercent: progress.percent, bytesPerSecond: progress.bytesPerSecond, error: undefined });
  });
  autoUpdater.on('update-downloaded', (info) => {
    setUpdate({ status: 'downloaded', availableVersion: info.version, progressPercent: 100, error: undefined });
    if (settings.notificationsEnabled && Notification.isSupported()) {
      const notification = new Notification({ title: 'Subutai шинэчлэгдлээ', body: 'Шинэ хувилбарыг суулгахад бэлэн.' });
      notification.on('click', showMainWindow);
      notification.show();
    }
  });
  autoUpdater.on('error', (error) => setUpdate({ status: 'error', error: error.message }));
}

async function initialize(): Promise<void> {
  store = new JobStore(join(app.getPath('userData'), 'data', 'subutai.db'));
  settings = normalizeSystemSettings(DEFAULT_SYSTEM_SETTINGS, store.loadState<Partial<SystemSettings>>(SETTINGS_KEY) ?? {});
