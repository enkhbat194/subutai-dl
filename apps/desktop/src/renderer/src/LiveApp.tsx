...
function ProgressBar({ job }: { job: DownloadJob }): ReactElement {
  ...
  return (
    <div className="progress-track" aria-label={`${progress}%`}>
      <span
        className={job.status === 'completed' ? 'progress-value progress-complete' : 'progress-value'}
    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
      <form className="new-download-modal" onSubmit={(event) => void submit(event)} onMouseDown={(event) => event.stopPropagation()}>
        <div className="modal-title-row">
          <div><span className="eyebrow">ARIA2 ТАТАЛТ</span><h2>Шинэ таталт нэмэх</h2></div>
        ...
        <div className="modal-grid">
          <label>Файлын нэр<input value={filename} onChange={(event) => setFilename(event.target.value)} placeholder="URL-аас автоматаар авна" /></label>
        ...
        <div className="modal-actions">
          ...
          <button type="submit" className="button primary" disabled={submitting || !url.trim()}>{submitting ? 'Холбож байна…' : 'Татаж эхлэх'}</button>
        </div>
      </form>
    </div>
  );
...
export function LiveApp(): ReactElement {
  ...
  const ariaHealthClass = health?.aria2.running ? 'good' : health?.aria2.error ? 'bad' : 'idle';
  const ariaHealthText = health?.aria2.running
    ? `aria2 ${health.aria2.version ?? ''} ажиллаж байна`
    : health?.aria2.error
      ? 'aria2 эхэлсэнгүй'
      : 'aria2 таталт эхлэхэд асна';
  ...
  return (
    <div className="app-shell">
      <header className="titlebar">
        ...
        <div className="titlebar-description">Subutai IDM — aria2 хөдөлгүүртэй бодит олон хэсэгт таталт<br />Pause, Resume, Cancel, Speed, ETA шууд шинэчлэгдэнэ.</div>
      ...
      <main className="dashboard">
        <section className="primary-grid">
          <aside className="left-sidebar panel">
            ...
            <div className={`engine-health ${ariaHealthClass}`}><span className="health-dot" /><div><b>{ariaHealthText}</b><small>{health?.aria2.executable ?? 'aria2c'}</small></div></div>
          </aside>
          ...
          <Panel className="downloads-panel">
            ...
            <div className="download-table">
              ...
              <div className="download-table-body">
                {visibleDownloads.length === 0 ? <div className="empty-downloads"><b>Одоогоор таталт алга.</b><span>“Шинэ таталт” дарж URL оруулна уу.</span></div> : visibleDownloads.map((job) => {
                  const progress = progressOf(job);
                  return <button key={job.id} className={selected?.id === job.id ? 'download-row selected' : 'download-row'} onClick={() => setSelectedId(job.id)}>
                    <span className="download-name"><FileGlyph filename={job.filename} /><b>{job.filename}</b></span>
                    <span>{formatBytes(job.totalBytes)}</span>
                    <span className="row-progress"><b>{progress}%</b><ProgressBar job={job} /></span>
                    <span>{formatSpeed(job.speedBytesPerSecond)}</span>
                    <span className={`status-text status-${job.status}`}>{formatEta(job.etaSeconds, job.status)}</span>
                  </button>;
                })}
              </div>
            </div>
          ...
          <Panel title="ТАТАЛТЫН ДЭЛГЭРЭНГҮЙ" className="detail-panel">
            {selected ? <>
              <div className="selected-file-title"><FileGlyph filename={selected.filename} /><span>{selected.filename}</span></div>
              <div className="tabs">{['Ерөнхий', 'Хэсгүүд', 'Холболтууд', 'Лог'].map((tab) => <button key={tab} className={detailTab === tab ? 'active' : ''} onClick={() => setDetailTab(tab)}>{tab}</button>)}</div>
              <div className="detail-overview">
                <div className="progress-ring" style={{ background: `conic-gradient(#2f8cff ${selectedProgress * 3.6}deg, #14243a 0deg)` }}><div><strong>{selectedProgress}%</strong><span>{formatBytes(selected.downloadedBytes)} / {formatBytes(selected.totalBytes)}</span></div></div>
                <dl><div><dt>Төлөв:</dt><dd>{statusLabel(selected.status)}</dd></div><div><dt>Хурд:</dt><dd>{formatSpeed(selected.speedBytesPerSecond)}</dd></div><div><dt>Үлдсэн хугацаа:</dt><dd>{formatEta(selected.etaSeconds, selected.status)}</dd></div><div><dt>Холболт:</dt><dd>{selected.connections}</dd></div><div><dt>GID:</dt><dd>{selected.engineTaskId ?? 'Хүлээгдэж байна'}</dd></div><div><dt>Хадгалах зам:</dt><dd>{selected.destination}</dd></div><div><dt>URL:</dt><dd className="truncate">{selected.url}</dd></div></dl>
              </div>
              <ProgressBar job={selected} />
              {selected.error ? <div className="action-error detail-error">{selected.error}</div> : null}
              <div className="segments-table"><div className="segment-head"><span>#</span><span>Холболт</span><span>Төлөв</span><span>Хэсгийн явц</span><span>Нийт хурд</span></div>{Array.from({ length: Math.max(1, Math.min(selected.connections, 8)) }, (_, index) => <div className="segment-row" key={index}><span>{index + 1}</span><span>aria2 connection {index + 1}</span><span>{statusLabel(selected.status)}</span><span>{selectedProgress}%</span><span>{formatSpeed(selected.speedBytesPerSecond)}</span></div>)}</div>
            </> : <div className="empty-detail"><b>Таталт сонгоогүй байна</b><span>Шинэ URL нэмэхэд дэлгэрэнгүй мэдээлэл энд гарна.</span><button className="button primary compact" onClick={() => setShowNewDownload(true)}>＋ Шинэ таталт</button></div>}
          </Panel>
        ...
        <section className="secondary-grid">
          <Panel title="ТӨЛӨВЛӨЛТ" className="schedule-panel"><div className="schedule-layout"><div className="schedule-list"><div className="schedule-head"><span>Нэр</span><span>Хугацаа</span><span>Төлөв</span></div>{[['Шөнийн таталт', '22:00 - 06:00', true], ['Ажлын өдрийн таталт', '09:00 - 18:00', false], ['Тогтмол таталт', 'Даваа, Лхагва, Баасан', false]].map(([name, time, enabled]) => <div className="schedule-row" key={String(name)}><b>{name}</b><span>{time}</span><span><Toggle active={Boolean(enabled)} />{enabled ? 'Идэвхтэй' : 'Идэвхгүй'}</span></div>)}</div><div className="schedule-form"><h3>Төлөвлөлтийн дэлгэрэнгүй</h3><label>Нэр:<input value="Шөнийн таталт" readOnly /></label><label>Эхлэх:<input value="22:00" readOnly /></label><label>Дуусах:<input value="06:00" readOnly /></label><label>Хурдны хязгаарлалт:<select defaultValue="Хязгаарлалтгүй"><option>Хязгаарлалтгүй</option></select></label></div></div></Panel>
          <Panel title="ФАЙЛЫН МЭДЭЭЛЭЛ" className="file-info-panel">{selected ? <><div className="file-info-title"><FileGlyph filename={selected.filename} /><b>{selected.filename}</b></div><dl className="file-info-list"><div><dt>Хэмжээ:</dt><dd>{formatBytes(selected.totalBytes)}</dd></div><div><dt>Татсан:</dt><dd>{formatBytes(selected.downloadedBytes)} ({selectedProgress}%)</dd></div><div><dt>Хэсэг:</dt><dd>{selected.connections}</dd></div><div><dt>Engine:</dt><dd>aria2</dd></div><div><dt>URL:</dt><dd className="link-text">{selected.url}</dd></div><div><dt>Хадгалах зам:</dt><dd>{selected.destination}</dd></div><div><dt>Үүссэн:</dt><dd>{new Date(selected.createdAt).toLocaleString()}</dd></div></dl><div className="panel-actions center"><button onClick={() => void runAction('folder')}>Нээх хавтас</button><button onClick={() => void navigator.clipboard.writeText(selected.url)}>URL хуулах</button></div></> : <div className="empty-mini">Файл сонгоно уу.</div>}</Panel>
          <Panel title="СТАТИСТИК" className="stats-panel"><div className="stat-main"><span>Энэ session-д татсан</span><strong>{formatBytes(downloads.reduce((sum, job) => sum + job.downloadedBytes, 0))}</strong></div><div className="chart"><div className="chart-grid" /><svg viewBox="0 0 300 110" preserveAspectRatio="none"><path d="M5 96 L50 90 L95 76 L140 79 L185 51 L230 42 L296 15 L296 108 L5 108 Z" fill="rgba(46,144,255,.2)"/><polyline points="5,96 50,90 95,76 140,79 185,51 230,42 296,15" fill="none" stroke="#3b9cff" strokeWidth="3" /></svg></div><dl className="stats-list"><div><dt>Нийт:</dt><dd>{downloads.length}</dd></div><div><dt>Амжилттай:</dt><dd>{completedCount}</dd></div><div><dt>Амжилтгүй:</dt><dd>{failedCount}</dd></div><div><dt>Татагдаж буй:</dt><dd>{activeCount}</dd></div><div><dt>Одоогийн хурд:</dt><dd>{formatSpeed(totalSpeed)}</dd></div></dl></Panel>
          <Panel title="ТОХИРГОО" className="settings-panel"><div className="settings-layout"><nav className="settings-nav"><button className="active">⚙ Ерөнхий</button><button>♧ Холболт</button><button>≋ Хурд</button><button>▥ Прокси</button><button>▱ Интерфейс</button></nav><div className="settings-content"><label className="check-row"><input type="checkbox" defaultChecked />Таталт дуусахад мэдэгдэх</label><label className="check-row"><input type="checkbox" defaultChecked />aria2-ийг автоматаар асаах</label><label>Үндсэн холболт:<select defaultValue="16"><option>4</option><option>8</option><option>16</option></select></label><label>Engine зам:<input value={health?.aria2.executable ?? 'aria2c'} readOnly /></label></div></div></Panel>
        </section>

        <section className="features-strip panel"><h2>ОНЦЛОГ, ДАВУУ ТАЛ</h2><div className="feature-grid">{[['ϟ', 'БОДИТ ТАТАЛТ', 'aria2 JSON-RPC-оос progress, speed, ETA авна.'], ['✣', 'ОЛОН ПРОТОКОЛ', 'HTTP, HTTPS, FTP, SFTP таталт.'], ['♢', 'НАЙДВАРТАЙ', 'Pause, Resume болон .aria2 recovery.'], ['◷', '750 MS SYNC', 'Таталтын төлөв UI-д шууд шинэчлэгдэнэ.'], ['◇', '16 ХОЛБОЛТ', 'aria2-ийн бодит connection limit.'], ['▱', 'WINDOWS READY', 'aria2c.exe-г resources/engines-д байршуулна.']].map(([icon, title, text]) => <article key={title}><span>{icon}</span><div><h3>{title}</h3><p>{text}</p></div></article>)}</div></section>
      </main>
